...
def test_add(mock_run, monkeypatch):
    ...
    mock_run.assert_called_with('( crontab -l | grep -ivF "echo hello"; '
    ...
    mock_run.assert_called_with('( crontab -l | grep -ivF "hello"; '
                                'echo "* * * * * PATH=/usr/local/bin:\$PATH echo hello" ) | crontab -',
                                shell=True, stderr=-2)
    ...
    mock_run.assert_called_with('( crontab -l | grep -ivF "echo hello"; echo "10 * * * * '
...
def test_remove(mock_run):
    ...
    mock_run.assert_called_with('( crontab -l | grep -ivF "hello" ) | crontab -',
                                shell=True, stderr=-2)
